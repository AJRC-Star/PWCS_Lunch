...
**Pattern to respect:** two successive refresh mechanisms have now disappeared silently — the Codex automation by 2026-07-04, the scheduled task by 2026-08-24. Neither announced its own death; both were caught only by the `Check Menu Freshness` workflow, and the second only once PWCS classes resumed on 2026-08-24 and lifted the summer-break suppression in `isPWCSSummerBreak`. That workflow is the project's only end-to-end detector. Treat its failures as true statements about the data until proven otherwise, and do not weaken, silence, or "fix" it to go green. As of 2026-08-26 the refresh-deadline check runs **year-round**: it only compares timestamps, never day content, so it is a pure liveness signal and is no longer skipped during quiet periods — only the plausibility check still is, because the artifact legitimately has no visible days then. A quiet-period failure is worded differently from an in-session one so a dead mechanism is distinguishable from merely stale data. The decision logic lives in `scripts/refresh-deadline.ts` rather than inline in the check script, deliberately: keeping it importable without a `process.argv[1]` run-guard means the watchdog cannot silently exit 0 having checked nothing.
