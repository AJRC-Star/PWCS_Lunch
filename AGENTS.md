...
- **The launchd fallback cannot run until `/bin/bash` is granted Full Disk Access.** This checkout lives in iCloud Drive (`~/Library/Mobile Documents`), which macOS protects with TCC. A launchd agent carries no TCC consent, so it can `stat` and `cd` into the repo but cannot read any file inside it; every run dies at once with `Operation not permitted` (exit 126). Verified 2026-08-08 that a wrapper script stored outside iCloud does **not** work around this — the restriction is on reading iCloud contents, not on where the invoked script lives. Grant access under System Settings > Privacy & Security > Full Disk Access, then reinstall and verify with `launchctl kickstart -k gui/$(id -u)/com.pwcs-lunch.fetch-menu`.
- Column 2 of `launchctl list | grep pwcs` is the last exit status: `0` success, `126` the TCC failure above, `78` launchd could not open the log paths. Agent logs live at `~/Library/Logs/pwcs-lunch/`, deliberately outside iCloud — launchd cannot create them inside the protected volume, and it fails before running anything, so no log explains why.
