...
**The No-Blur Rule.** `backdrop-filter` is used nowhere in this stylesheet. It was previously set on the header and tab strip, which never satisfied the condition that made it meaningful: neither is `position: sticky`, and `#app`'s flex column means nothing is ever painted behind them, so the filter had only the flat body colour to blur. Default to tonal layering; reach for blur only if an element is genuinely `position: sticky` or `fixed` with content passing underneath it.

Worth recording why it left, because the stated reason was wrong at the time: it was removed while chasing a blurred header title in the installed PWA, on the theory that the compositing layer it forces was being rasterised at the wrong scale. It was not the cause — see The Safe-Area Clearance Rule below — and removing it changed nothing. It stays out on the no-op grounds alone.

**The Safe-Area Clearance Rule.** `env(safe-area-inset-top)` marks where the iOS status bar ends, not where text becomes legible. `index.html` sets `apple-mobile-web-app-status-bar-style: black-translucent`, so an installed standalone PWA renders *underneath* the status bar and iOS blurs whatever sits in that band to keep the clock readable — and the band reaches past the inset iOS reports. With the header's `padding-top` at exactly the inset, the `h1` glyphs started ~10pt below it and still rendered visibly blurred while the sub-label 30px further down was sharp. Anything positioned against the top inset needs clearance beyond it: the header carries `calc(env(safe-area-inset-top) + 24px)`, and `.error-banner` carries `+ 12px` on top of its own 12px padding to land its text at the same offset. Do not trim either back to the bare inset. This only reproduces in an installed PWA, never in a Safari tab, so it cannot be caught in a desktop browser.
...
- **Do** keep text clear of `env(safe-area-inset-top)` by a margin, never flush against it. (See: The Safe-Area Clearance Rule.)
...
- **Don't** re-add `backdrop-filter` to the header, the tab strip, cards, or decorative surfaces. Nothing in this layout has anything painted behind it, so the effect cannot render. (See: The No-Blur Rule.)
